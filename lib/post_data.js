...
SlackDataPostHandler.prototype.postData = function(data) {
  var recipient, attachment_color, splitMessage,
      pretext = "";
  ...
  splitMessage = utils.splitMessage(this.formatter.formatData(data.message));

  if (splitMessage.text) {
    this.robot.emit('slack-attachment', {
      channel: recipient,
      text: pretext + splitMessage.pretext,
      content: {
    ...
    this.robot.messageRoom.call(this.robot, recipient, pretext + splitMessage.pretext);
  }
};
...
DefaultFormatter.prototype.postData = function(data) {
  var recipient, splitMessage, formattedMessage,
      text = "";
  ...
  splitMessage = utils.splitMessage(text);
  if (splitMessage.pretext && splitMessage.text) {
    formattedMessage = util.format("%s\n%s", splitMessage.pretext, splitMessage.text);
  } else {
    formattedMessage = splitMessage.pretext || spiltMessage.text;
  } 

  this.robot.messageRoom.call(this.robot, recipient, formattedMessage);
};
